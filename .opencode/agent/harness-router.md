...
4. Read `.opencode/memory/harness-router.json` before routing to detect patterns. If no history, proceed normally.
...
After returning the matrix, you MAY invoke `general` subagent to record the routing decision:
```
task(description="record in memory: route <scope> mode=<plan|build> required=<list>", subagent_type="general")
```
