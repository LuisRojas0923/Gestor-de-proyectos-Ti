...
- Bitácora actualizada registra:
  - `python -m pytest testing/backend/test_autogestion_usuarios_erp.py testing/backend/test_jit_approval.py testing/backend/test_jit_contrasena_eq_cedula.py -v` -> **21 passed**.
...
- Verificación permitida por este revisor:
  - `python -m pytest --collect-only testing/backend/test_autogestion_usuarios_erp.py testing/backend/test_jit_approval.py testing/backend/test_jit_contrasena_eq_cedula.py` -> **21 tests collected**.
...
- `docs/bitacora/2026-07-09-autogestion-usuarios-activos-erp.md` ya refleja **21 passed** y conserva los bloqueos locales de `test_setup_password.py` y `npm run lint`.
...
- Ya no se requiere una prueba adicional bloqueante para el happy-path de `/auth/setup-password` sin usuario local; está cubierta por `test_setup_password_sin_usuario_local_erp_activo_crea_usuario_habilitado` y respaldada por la evidencia **21 passed**.
...
El bloqueo docs/tests previo queda cerrado por la nueva prueba happy-path y la bitácora actualizada a **21 passed**. Persisten solo riesgos de validación ambiental (`test_setup_password.py` y lint frontend), ya trazados y no bloqueantes para esta revisión.
