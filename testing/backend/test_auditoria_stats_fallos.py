...
async def test_obtener_estadisticas_clasificacion_fallos_detallados(db_session):
    # La fixture canónica apunta exclusivamente a la base configurada para pruebas.

    # Obtener conteo inicial
    stats_inicial = await ServicioAuditoriaEstadisticas.obtener_estadisticas(db_session)
    fallos_map_inicial = {
        f["tipo"]: f["total"] for f in stats_inicial.get("tipos_fallos", [])
    }
    ...
    assert (
        detalles_valid.get(msg_valid, 0) - detalles_valid_inicial.get(msg_valid, 0) == 1
    )
