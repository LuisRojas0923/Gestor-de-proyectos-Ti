...
from app.api.lineas_corporativas import alertas_router
...
@pytest.mark.asyncio
async def test_alertas_empleados_degrada_si_erp_no_responde(monkeypatch):
    db_local = AsyncMock()
    resultado = MagicMock()
    resultado.scalars.return_value.all.return_value = ["1107068093"]
    db_local.execute.return_value = resultado
    db_erp = MagicMock()
    ejecutar_en_hilo = AsyncMock(side_effect=RuntimeError("ERP fuera de servicio"))
    monkeypatch.setattr(alertas_router, "run_in_threadpool", ejecutar_en_hilo, raising=False)

    respuesta = await obtener_alertas_empleados(db_erp, db_local)

    assert respuesta == {"error": "ERP no disponible", "alertas": {}}
    ejecutar_en_hilo.assert_awaited_once()
    db_erp.execute.assert_not_called()
