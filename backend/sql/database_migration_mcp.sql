-- =====================================================================================
-- MIGRACIÓN MCP - Model Context Protocol para IA
...
WHERE aum.user_id IS NOT NULL
GROUP BY u.name, u.email, aum.ai_model;

COMMENT ON VIEW ai_user_usage_summary IS 'Resumen de uso de IA por usuario y modelo';
...
SELECT 
    ...
    ROUND(
        COUNT(CASE WHEN status IN ('accepted', 'implemented') THEN 1 END) * 100.0 / COUNT(*), 2
    ) as acceptance_rate,
...
COMMENT ON VIEW ai_recommendation_effectiveness IS 'Efectividad y adopción de recomendaciones por tipo';

-- Vista de Análisis de Costos de IA
...
GROUP BY DATE_TRUNC('month', created_at), ai_model, operation_type
ORDER BY month DESC, total_cost DESC;

COMMENT ON VIEW ai_cost_analysis IS 'Análisis mensual de costos y uso por modelo y operación';

-- Vista de Cache Hit Rate
CREATE VIEW ai_cache_performance AS
SELECT 
    context_type,
    COUNT(*) as total_contexts,
    AVG(access_count) as avg_access_count,
    SUM(access_count) as total_accesses,
    COUNT(CASE WHEN access_count > 1 THEN 1 END) as reused_contexts,
    ROUND(
        COUNT(CASE WHEN access_count > 1 THEN 1 END) * 100.0 / COUNT(*), 2
    ) as cache_hit_rate,
    AVG(EXTRACT(EPOCH FROM (expires_at - created_at))) as avg_ttl_seconds
FROM ai_context_cache
GROUP BY context_type;

COMMENT ON VIEW ai_cache_performance IS 'Performance del cache de contexto por tipo';
...
-- Crear rol específico para servicios de IA
-- CREATE ROLE ai_service_role;

-- Permisos para tablas MCP
-- GRANT SELECT, INSERT, UPDATE ON ai_context_cache TO ai_service_role;
-- GRANT SELECT, INSERT ON ai_analysis_history TO ai_service_role;
-- GRANT SELECT, INSERT, UPDATE ON ai_recommendations TO ai_service_role;
-- GRANT SELECT, INSERT ON ai_usage_metrics TO ai_service_role;
-- GRANT SELECT ON ai_model_configs TO ai_service_role;

-- Permisos para vistas
-- GRANT SELECT ON ai_user_usage_summary TO ai_service_role;
-- GRANT SELECT ON ai_recommendation_effectiveness TO ai_service_role;
-- GRANT SELECT ON ai_cost_analysis TO ai_service_role;
-- GRANT SELECT ON ai_cache_performance TO ai_service_role;

-- Permisos para funciones
-- GRANT EXECUTE ON FUNCTION clean_expired_ai_cache() TO ai_service_role;
-- GRANT EXECUTE ON FUNCTION get_ai_usage_stats(DATE, DATE) TO ai_service_role;
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            RAISE EXCEPTION '❌ Error: Tabla % no fue creada', table_name;
        END IF;
    END LOOP;
