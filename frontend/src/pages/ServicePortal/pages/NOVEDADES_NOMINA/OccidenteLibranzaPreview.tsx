...
interface OccidenteLibranzaRow {
    cedula: string;
...
interface OccidenteLibranzaResponse {
    rows: OccidenteLibranzaRow[];
    summary: {
...
const OccidenteLibranzaPreview: React.FC = () => {
    ...
    const handleDesvincular = async (registroId: number | string) => {
        ...
        try {
            await axios.delete(`${API_CONFIG.BASE_URL}/novedades-nomina/excepciones/vincular-dinamico`, {
                data: { cedula, concepto, mes, anio, subcategoria: "OCCIDENTE" }
            });
            setData({
                ...data,
                rows: data.rows.map(r => 
    ...
    const [data, setData] = useState<OccidenteLibranzaResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/occidente_libranza/datos`,
                    { params: { mes, anio } }
                );
                ...
                console.error('Error cargando datos OCCIDENTE LIBRANZA:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/occidente_libranza/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
    ...
    const getColumnOptions = React.useCallback((key: keyof OccidenteLibranzaRow) => {
        if (!data) return [];
            ...
            <div className="flex-none pb-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <Title variant="h4" weight="bold">OCCIDENTE LIBRANZA – Preview</Title>
                            <Text color="text-secondary">LIBRANZAS / OCCIDENTE LIBRANZA</Text>
                        </div>
                    </div>
                    <Button 
                        variant="ghost" 
                        ...
                        onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=OCCIDENTE LIBRANZA')}
                    >
...
export default OccidenteLibranzaPreview;
