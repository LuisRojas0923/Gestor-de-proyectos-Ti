...
const RetencionesPreview: React.FC = () => {
    ...
    const handleDesvincular = async (registroId: number | string) => {
        ...
        try {
            await axios.delete(`${API_CONFIG.BASE_URL}/novedades-nomina/excepciones/vincular-dinamico`, {
                data: { cedula, concepto, mes, anio, subcategoria: "RETENCIONES" }
            });
            setData({
                ...data,
                rows: data.rows.map(r => 
    ...
    const getColumnOptions = React.useCallback((key: keyof RetencionesRow) => {
        if (!data || !data.rows) return [];
                                    ...
                                    <table className="w-full text-[10px]">
                                        ...
                                        <tbody className="divide-y divide-amber-100">
                                            {warningsDetalle.map((w: WarningDetalle, i: number) => (
                                                <tr onContextMenu={(e) => handleContextMenu(e, row)} key={`${w.cedula || 'warn'}-${i}`} className={`hover:bg-amber-50 cursor-context-menu ${row.estado_erp === "EXCEPTUADO" ? "bg-slate-100/80 opacity-60 line-through" : ""}`}>
