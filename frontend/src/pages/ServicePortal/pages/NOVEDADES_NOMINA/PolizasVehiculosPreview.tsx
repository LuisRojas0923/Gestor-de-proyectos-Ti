...
const PolizasVehiculosPreview: React.FC = () => {
    ...
    const handleDesvincular = async (registroId: number | string) => {
        ...
        try {
            await axios.delete(`${API_CONFIG.BASE_URL}/novedades-nomina/excepciones/vincular-dinamico`, {
                data: { cedula, concepto, mes, anio, subcategoria: "POLIZAS" }
            });
            setData({
                ...data,
                rows: data.rows.map(r => 
    ...
    const getColumnOptions = React.useCallback((key: keyof PolizasVehiculosRow) => {
        if (!data) return [];
                ...
                <div className="flex flex-col md:flex-row items-end gap-4">
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                        ...
                        <div className="relative group">
                            <input id="file-upload-polizas" // @audit-ok
                                type="file"
                                multiple
                                accept=".xlsx,.xls"
                                    ...
                                    <table className="w-full text-[10px]">
                                        ...
                                        <tbody className="divide-y divide-red-100">
                                            {warningsDetalle.map((w: WarningDetalle, i: number) => (
                                                <tr key={`\${w.cedula || \'warn\'}-\${i}`} className="hover:bg-red-50 transition-colors">
