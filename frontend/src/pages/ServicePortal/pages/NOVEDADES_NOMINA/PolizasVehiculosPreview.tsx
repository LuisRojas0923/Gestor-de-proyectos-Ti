...
interface PolizasVehiculosResponse {
    rows: PolizasVehiculosRow[];
    summary: {
...
const PolizasVehiculosPreview: React.FC = () => {
    ...
    const [data, setData] = useState<PolizasVehiculosResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/polizas-vehiculos/datos`,
                    { params: { mes, anio } }
                );
                if (res.data.rows && res.data.rows.length > 0) {
                    setData({ rows: res.data.rows, summary: res.data.summary, warnings: [], warnings_detalle: [] });
                ...
                console.error('Error cargando datos PÓLIZAS VEHÍCULOS:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/polizas-vehiculos/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
            addNotification('success', `Procesados ${res.data.summary.total_asociados} registros.`);
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500 pb-12">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <Title variant="h4" weight="bold">PÓLIZAS VEHÍCULOS – Preview</Title>
                    <Text color="text-secondary">SEGUROS / PÓLIZAS VEHÍCULOS</Text>
                </div>
            </div>

            {/* Historial */}
            <div
                onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=POLIZAS VEHICULOS')}
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <Text size="sm" color="text-secondary">Ver archivos cargados anteriormente para PÓLIZAS VEHÍCULOS</Text>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 space-y-6">
                ...
                >
                    {isProcessing ? (
                        ...
                        <Text as="span" color="inherit" className="flex items-center gap-2">
                            <FileText className="w-4 h-4" /> Procesar Archivo
                        </Text>
                    )}
                </Button>
            </div>
            ...
            {data && !isLoading && (
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <SummaryCard label="Registros" value={data.summary.total_asociados} />
                        ...
                        <SummaryCard label="Archivos" value={data.summary.archivos_procesados ?? 1} />
                    </div>
                    ...
                    {warningsDetalle.length > 0 && (
                        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-2xl p-6 space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="outline" size="sm" onClick={() => setShowWarnings(!showWarnings)}>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Select
                                value=""
                                onChange={() => {}}
                                options={[{ value: '', label: 'PÓLIZA VEHÍCULOS VALOR' }]}
                            />
                        </div>
                    </div>
                        ...
                        <div className="overflow-x-auto overflow-y-auto max-h-[600px]">
                            <table className="w-full text-sm border-collapse">
                                <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-900 shadow-sm">
                                    <tr>
                                        <th className="text-left p-4 font-semibold text-slate-600 dark:text-slate-300 bg-inherit whitespace-nowrap">#</th>
                                        ...
                                        <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                            ...
                                            <td className="p-4">
                                                ...
                                                {row.estado_erp === 'RETIRADO' && (
                                                    <span className="ml-2 inline-flex items-center gap-1 text-xs text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full font-semibold">
                                            ...
                                            <td className="p-4">
                                                {row.empresa ? (
                                                    ...
                                                    <span className="text-red-600 dark:text-red-400 font-semibold uppercase text-xs">
                                                        RETIRADO
                                                    </span>
                                                )}
                                            </td>
                                            ...
                                            <td className="p-4">
                                                <Badge variant="info" size="sm">{row.concepto}</Badge>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
...
export default PolizasVehiculosPreview;
