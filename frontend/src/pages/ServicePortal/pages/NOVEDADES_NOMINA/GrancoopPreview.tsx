...
const GrancoopPreview: React.FC = () => {
    ...
    const [data, setData] = useState<GrancoopResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/grancoop/datos`,
                    { params: { mes, anio } }
                );
                ...
                console.error('Error cargando datos GRANCOOP:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/grancoop/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
    ...
    return (
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500 pb-12">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <Title variant="h4" weight="bold">GRANCOOP – Preview</Title>
                    <Text color="text-secondary">COOPERATIVAS / GRANCOOP</Text>
                </div>
            </div>

            {/* Historial de GRANCOOP */}
            <div
                onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=GRANCOOP')}
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <Text size="sm" color="text-secondary">Ver archivos cargados anteriormente para GRANCOOP</Text>
                </div>
...
const ConceptoBadge: React.FC<{ concepto: string }> = ({ concepto }) => {
    const variantMap: Record<string, 'success' | 'info' | 'warning' | 'default'> = {
        'GRANCOOP APORTES': 'success',
        'GRANCOOP PRESTAMOS': 'info',
        'GRANCOOP ADICIONALES': 'warning',
    };
    return (
...
export default GrancoopPreview;
