...
interface DaviviendaLibranzaResponse {
    rows: DaviviendaLibranzaRow[];
    summary: {
...
const DaviviendaLibranzaPreview: React.FC = () => {
    ...
    const handleDesvincular = async (registroId: number | string) => {
        ...
        try {
            await axios.delete(`${API_CONFIG.BASE_URL}/novedades-nomina/excepciones/vincular-dinamico`, {
                data: { cedula, concepto, mes, anio, subcategoria: "DAVIVIENDA LIBRANZA" }
            });
            setData({
                ...data,
                rows: data.rows.map(r => 
    ...
    const [data, setData] = useState<DaviviendaLibranzaResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/l_davivienda/datos`,
                    { params: { mes, anio } }
                );
                ...
                console.error('Error cargando datos DAVIVIENDA LIBRANZA:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/l_davivienda/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
    ...
    const getColumnOptions = React.useCallback((key: keyof DaviviendaLibranzaRow) => {
        if (!data) return [];
    ...
    return (
        <div className="max-w-[1600px] mx-auto h-[calc(100vh-170px)] flex flex-col animate-in fade-in duration-500 overflow-hidden px-1 space-y-2">
            ...
            <div className="flex-none pb-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <Title variant="h4" weight="bold">DAVIVIENDA LIBRANZA – Preview</Title>
                            <Text color="text-secondary">LIBRANZAS / DAVIVIENDA LIBRANZA</Text>
                        </div>
                    </div>
                    <Button 
                        variant="ghost" 
                        ...
                        onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=DAVIVIENDA LIBRANZA')}
                    >
            ...
            <ModalVincularExcepcion
                ...
                mes={mes}
                anio={anio}
                subcategoria="DAVIVIENDA LIBRANZA"
...
export default DaviviendaLibranzaPreview;
