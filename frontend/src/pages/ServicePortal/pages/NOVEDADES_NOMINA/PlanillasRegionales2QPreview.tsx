        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/planillas_regionales_2q/datos`,
                    { params: { mes, anio } }
                );
                ...
                console.error('Error cargando datos PLANILLAS REGIONALES 2Q:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/planillas_regionales_2q/preview`,
                formData,
            );
            if (res.data.error) {
            ...
            <div className="flex-none pb-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <Title variant="h4" weight="bold">PLANILLAS REGIONALES 2Q – Preview</Title>
                            <Text color="text-secondary">NOVEDADES / PLANILLAS REGIONALES 2Q</Text>
                        </div>
                    </div>
                    <Button 
                        variant="ghost" 
                        ...
                        onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=PLANILLAS REGIONALES 2Q')}
                    >
                                        ...
                                        <table className="w-full text-xs">
                                            ...
                                            <tbody className="divide-y divide-amber-100 dark:divide-amber-800">
                                                {warningsDetalle.map((w, i) => (
                                                    <tr onContextMenu={(e) => handleContextMenu(e, row)} key={`${w.cedula || 'warn'}-${i}`} className={`hover:bg-amber-50 dark:hover:bg-amber-900/20 cursor-context-menu ${row.estado_erp === "EXCEPTUADO" ? "bg-slate-100/80 opacity-60 line-through" : ""}`}>
                                ...
                                <table className="w-full text-[11px] border-collapse">
                                    <thead className="sticky top-0 z-10">
                                        <tr className="bg-[var(--color-primary-900)] text-white shadow-md">
                                            ...
                                            <th className="text-center py-2 px-4 font-bold uppercase tracking-wider w-32 border-b border-white/5 border-r border-white/5"><Text as="span" size="xs" color="inherit">DIAS</Text></th>
