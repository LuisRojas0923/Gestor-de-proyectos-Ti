...
const BeneficiarPreview: React.FC = () => {
    ...
    const [data, setData] = useState<BeneficiarResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/beneficiar/datos`,
                    { params: { mes, anio } }
                );
                ...
                console.error('Error cargando datos BENEFICIAR:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/beneficiar/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
    ...
    return (
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500 pb-12">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <Title variant="h4" weight="bold">BENEFICIAR – Preview</Title>
                    <Text color="text-secondary">COOPERATIVAS / BENEFICIAR</Text>
                </div>
            </div>

            {/* Historial */}
            <div
                onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=BENEFICIAR')}
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <Text size="sm" color="text-secondary">Ver archivos cargados anteriormente para BENEFICIAR</Text>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 space-y-6">
                ...
                >
                    {isProcessing ? (
                        ...
                        <Text as="span" color="inherit" className="flex items-center gap-2">
                            <FileText className="w-4 h-4" /> Procesar Archivos
                        </Text>
                    )}
                </Button>
            </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Select
                                ...
                                options={[
                                    ...
                                    { value: 'BENEFICIAR APORTE', label: 'BENEFICIAR APORTE' },
                                    { value: 'BENEFICIAR CREDITO', label: 'BENEFICIAR CREDITO' },
                                    { value: 'BENEFICIAR OTROS DESCUENTOS', label: 'BENEFICIAR OTROS DESCUENTOS' },
                                ]}
                            />
                        </div>
                    </div>
...
export default BeneficiarPreview;
