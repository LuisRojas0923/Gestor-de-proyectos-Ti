...
interface BogotaLibranzaResponse {
    rows: BogotaLibranzaRow[];
    summary: {
...
const BogotaLibranzaPreview: React.FC = () => {
    ...
    const [data, setData] = useState<BogotaLibranzaResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/bogota_libranza/datos`,
                    { params: { mes, anio } }
                );
                ...
                console.error('Error cargando datos BOGOTA LIBRANZA:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/bogota_libranza/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
    ...
    const getColumnOptions = React.useCallback((key: keyof BogotaLibranzaRow) => {
        if (!data) return [];
            ...
            <div className="flex-none pb-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <Title variant="h4" weight="bold">BOGOTA LIBRANZA – Preview</Title>
                            <Text color="text-secondary">LIBRANZAS / BOGOTA LIBRANZA</Text>
                        </div>
                    </div>
                    <Button 
                        variant="ghost" 
                        ...
                        onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=BOGOTA LIBRANZA')}
                    >
...
export default BogotaLibranzaPreview;
