...
const RecordarPreview: React.FC = () => {
    ...
    const handleDesvincular = async (registroId: number | string) => {
        ...
        try {
            await axios.delete(`${API_CONFIG.BASE_URL}/novedades-nomina/excepciones/vincular-dinamico`, {
                data: { cedula, concepto, mes, anio, subcategoria: "RECORDAR" }
            });
            setData({
                ...data,
                rows: data.rows.map(r => 
    ...
    const [data, setData] = useState<RecordarResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/recordar/datos`,
                    { params: { mes, anio } }
                );
                const hasRows = res.data.rows && res.data.rows.length > 0;
                const hasWarnings = res.data.warnings_detalle && res.data.warnings_detalle.length > 0;

                if (hasRows || hasWarnings) {
                    setData({ 
                        rows: res.data.rows || [], 
                        summary: { ...res.data.summary, archivos_procesados: res.data.summary.archivos_procesados || 1 }, 
                        warnings: [], 
                        warnings_detalle: [] 
                    });
                ...
                console.error('Error cargando datos RECORDAR:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/recordar/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
    ...
    const getColumnOptions = React.useCallback((key: keyof RecordarRow) => {
        if (!data) return [];
            ...
            <div className="flex-none pb-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <Title variant="h4" weight="bold">RECORDAR – Preview</Title>
                            <Text color="text-secondary">FUNEBRES / RECORDAR</Text>
                        </div>
                    </div>
                    <Button 
                        variant="ghost" 
                        ...
                        onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=RECORDAR')}
                    >
                        ...
                        <div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-slate-500 font-bold">
                            <Text as="span" size="xs" color="inherit" className="font-bold">RECORDAR / HISTORIAL</Text>
                            ...
                            <div className="flex items-center gap-2 px-4 h-[42px] rounded-xl border border-dashed border-slate-300 dark:border-slate-600 group-hover:border-[var(--color-primary)] transition-colors cursor-pointer bg-slate-50 dark:bg-slate-900/50">
                                ...
                                <Text size="sm" color="text-secondary" className="truncate">
                                    {files.length > 0
                                        ...
                                        : 'Seleccionar archivos CSV/Excel...'}
                                </Text>
                            </div>
                        </div>
                    </div>
...
export default RecordarPreview;
