...
interface ControlDescuentosRow {
    cedula: string;
...
interface ControlDescuentosResponse {
    rows: ControlDescuentosRow[];
    summary: {
...
const ControlDescuentosPreview: React.FC = () => {
    ...
    const [data, setData] = useState<ControlDescuentosResponse | null>(null);
        ...
        const fetchSaved = async () => {
            ...
            try {
                const res = await axios.get(
                    `${API_CONFIG.BASE_URL}/novedades-nomina/control_descuentos/datos`,
                    { params: { mes, anio } }
                );
                ...
                console.error('Error cargando datos CONTROL DE DESCUENTOS:', err);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${API_CONFIG.BASE_URL}/novedades-nomina/control_descuentos/preview`,
                formData,
            );
            setData(res.data);
            setWarningsDetalle(res.data.warnings_detalle || []);
    ...
    const getColumnOptions = (column: keyof ControlDescuentosRow) => {
        if (!data) return [];
        const uniqueValues = new Set(data.rows.map(r => String(r[column] || 'N/A')));
            ...
            .filter(r => {
                ...
                for (const col in activeFilters) {
                    ...
                    if (selectedValues.length > 0) {
                        const val = String(r[col as keyof ControlDescuentosRow] || 'N/A');
            ...
            <div className="flex-none pb-1">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <Title variant="h4" weight="bold">CONTROL DE DESCUENTOS – Preview</Title>
                            <Text color="text-secondary">DESCUENTOS / CONTROL DE DESCUENTOS</Text>
                        </div>
                    </div>
                    <Button 
                        variant="ghost" 
                        ...
                        onClick={() => navigate('/service-portal/novedades-nomina/historial?subcategoria=CONTROL DE DESCUENTOS')}
                    >
                            ...
                            <table className="w-full text-[11px] border-collapse">
                                <thead className="sticky top-0 z-10">
                                    <tr className="bg-[var(--color-primary-900)] text-white shadow-md">
                                        ...
                                        <th className="text-right py-3 px-4 font-bold uppercase tracking-wider w-36 border-b border-white/5 border-r border-white/5">
...
export default ControlDescuentosPreview;
