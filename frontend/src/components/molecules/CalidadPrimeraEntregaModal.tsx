...
import { CalidadPrimeraEntregaDetail, CalidadPrimeraEntregaResponse } from '../../hooks/useKpiDetails';
...
const CalidadPrimeraEntregaModal: React.FC<CalidadPrimeraEntregaModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className={`${
      ...
      } rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-hidden`}>
        {/* Header */}
        <div className={`${
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className={`${
              darkMode ? 'bg-neutral-700 border-neutral-600' : 'bg-blue-50 border-blue-200'
            } border rounded-lg p-4`}>
              <p className={`text-sm font-medium ${
                darkMode ? 'text-neutral-400' : 'text-blue-600'
              }`}>
                Total Entregas
              </p>
            ...
            <div className={`${
              darkMode ? 'bg-neutral-700 border-neutral-600' : 'bg-green-50 border-green-200'
            } border rounded-lg p-4`}>
              <p className={`text-sm font-medium ${
                darkMode ? 'text-neutral-400' : 'text-green-600'
              }`}>
                Sin Devoluciones
              </p>
              <p className={`text-2xl font-bold ${
            ...
            <div className={`${
              darkMode ? 'bg-neutral-700 border-neutral-600' : 'bg-red-50 border-red-200'
            } border rounded-lg p-4`}>
              <p className={`text-sm font-medium ${
                darkMode ? 'text-neutral-400' : 'text-red-600'
              }`}>
                Con Devoluciones
              </p>
              <p className={`text-2xl font-bold ${
            ...
            <div className={`${
              darkMode ? 'bg-neutral-700 border-neutral-600' : 'bg-purple-50 border-purple-200'
            } border rounded-lg p-4`}>
              <p className={`text-sm font-medium ${
                darkMode ? 'text-neutral-400' : 'text-purple-600'
              }`}>
                Calidad
              </p>
              <p className={`text-2xl font-bold ${
          ...
          <div className={`${
            ...
            <div className={`${
              darkMode ? 'bg-neutral-600' : 'bg-gray-50'
            } px-6 py-3 border-b`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className={`${
                  ...
                  <tr>
                    <th className={`px-4 py-3 text-left text-xs font-medium ${
                      darkMode ? 'text-neutral-300' : 'text-gray-500'
                ...
                <tbody className={`${
                  ...
                  darkMode ? 'divide-neutral-600' : 'divide-gray-200'
                }`}>
                  {data.details.map((detail, index) => (
                    <tr key={`${detail.development_id}-${index}`}>
                      ...
                      <td className="px-4 py-4">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(detail.estado_calidad)}`}>
