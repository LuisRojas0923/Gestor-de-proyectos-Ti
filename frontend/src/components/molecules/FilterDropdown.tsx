...
export const FilterDropdown: React.FC<FilterDropdownProps> = (props) => {
    ...
    // Unificar anchorRect (en coordenadas del viewport para posicionamiento fixed)
    let effectiveAnchor: { top: number; left: number; width: number } | null = null;
    if (isSimpleMode) {
        effectiveAnchor = triggerRect;
    } else if (anchorRect) {
        effectiveAnchor = {
            top: anchorRect.top,
    ...
    const dynamicStyle: React.CSSProperties = {
        ...
        left: position && effectiveAnchor ? Math.min(position.left, window.innerWidth - Math.max(effectiveAnchor.width, 280) - 20) : 0,
        width: effectiveAnchor ? Math.max(effectiveAnchor.width, 280) : 280,
        maxHeight: maxHeight
    };
    ...
    return (
        <>
            ...
            {effectiveIsOpen && effectiveAnchor && position && createPortal(
                <div
                    ref={dropdownRef}
