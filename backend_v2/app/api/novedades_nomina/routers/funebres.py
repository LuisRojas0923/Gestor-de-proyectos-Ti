...
async def preview_camposanto(
    ...
    # Cedulas que no deben buscarse en ERP (ya son excepciones o informativas)
    ...
    for row in rows:
        ...
        if ced in mapa_excepciones:
            ...
            # 2. PAGO_TERCERO y tiene un pagador_cedula, redireccionamos
    ...
    try:
        ...
        for idx, row in enumerate(rows_facturables):
            reg = NominaRegistroNormalizado(
                ...
                estado_validacion="OK" if str(row.get("estado_erp")).upper() == "ACTIVO" else row.get("estado_erp", "OK"), 
        ...
        for idx, row in enumerate(rows_no_facturables):
            reg = NominaRegistroNormalizado(archivo_id=archivo.id, fecha_creacion=datetime.now(), mes_fact=mes, año_fact=anio, cedula=row["cedula"], nombre_asociado=row.get("nombre_asociado", ""), valor=row["valor"], empresa=row.get("empresa", ""), concepto=row["concepto"], categoria_final="FUNEBRES", subcategoria_final="CAMPOSANTO", estado_validacion=row.get("estado_erp", "ADVERTENCIA"), observaciones=row.get("observaciones"), fila_origen=len(rows_facturables) + idx + 1)
            session.add(reg)
...
async def obtener_datos_camposanto(mes: int = Query(...), anio: int = Query(...), session: AsyncSession = Depends(obtener_db)):
    """Devuelve datos CAMPOSANTO guardados para un mes/año."""
    stmt = select(NominaRegistroNormalizado).where(NominaRegistroNormalizado.subcategoria_final == "CAMPOSANTO", NominaRegistroNormalizado.mes_fact == mes, NominaRegistroNormalizado.año_fact == anio)
    try:
    ...
    try:
        ...
        for idx, row in enumerate(rows_no_facturables):
            reg = NominaRegistroNormalizado(
                ...
                categoria_final="FUNEBRES", 
                subcategoria_final="RECORDAR", 
                estado_validacion=row.get("estado_erp", "ADVERTENCIA"), 
                observaciones=row.get("observaciones"), 
                fila_origen=len(rows_facturables) + idx + 1
            )
            session.add(reg)
...
async def obtener_datos_recordar(mes: int = Query(...), anio: int = Query(...), session: AsyncSession = Depends(obtener_db)):
    """Devuelve datos RECORDAR guardados para un mes/año."""
    stmt = select(NominaRegistroNormalizado).where(NominaRegistroNormalizado.subcategoria_final == "RECORDAR", NominaRegistroNormalizado.mes_fact == mes, NominaRegistroNormalizado.año_fact == anio)
    try:
        result = await session.execute(stmt)
        registros = result.scalars().all()
    except Exception as e:
