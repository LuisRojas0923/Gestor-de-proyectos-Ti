...
async def preview_grancoop(
    ...
    try:
        ...
        archivo = NominaArchivo(nombre_archivo=f"grancoop_{mes}_{anio}.pdf", hash_archivo=hashlib.md5(archivos_binarios[0][:1024]).hexdigest() if archivos_binarios else "none", tamaño_bytes=sum(len(b) for b in archivos_binarios), tipo_archivo="pdf", ruta_almacenamiento="memory", mes_fact=mes, año_fact=anio, categoria="COOPERATIVAS", subcategoria="GRANCOOP", estado="Procesado")
