...
router = APIRouter(tags=["Libranzas - Occidente"])

@router.post("/occidente_libranza/preview")
async def preview_occidente_libranza(mes: int = Form(...), anio: int = Form(...), files: List[UploadFile] = File(...), session: AsyncSession = Depends(obtener_db), db_erp = Depends(obtener_erp_db_opcional)):
    ...
    stmt_exc = select(NominaExcepcion).where(
        NominaExcepcion.subcategoria == "OCCIDENTE LIBRANZA",
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error al consultar excepciones para Occidente: {str(e)}")
    ...
    try:
        await session.execute(delete(NominaRegistroNormalizado).where(NominaRegistroNormalizado.subcategoria_final == "OCCIDENTE LIBRANZA", NominaRegistroNormalizado.mes_fact == mes, NominaRegistroNormalizado.año_fact == anio))
        archivo = NominaArchivo(nombre_archivo=f"occidente_libranza_{mes}_{anio}.xlsx", hash_archivo=hashlib.md5(archivos_binarios[0][:1024]).hexdigest() if archivos_binarios else "none", tamaño_bytes=sum(len(b) for b in archivos_binarios), tipo_archivo="xlsx", ruta_almacenamiento="memory", mes_fact=mes, año_fact=anio, categoria="LIBRANZAS", subcategoria="OCCIDENTE LIBRANZA", estado="Procesado")
        session.add(archivo); await session.flush()
        for idx, row in enumerate(rows_facturables):
            reg = NominaRegistroNormalizado(archivo_id=archivo.id, fecha_creacion=datetime.now(), mes_fact=mes, año_fact=anio, cedula=row["cedula"], nombre_asociado=row.get("nombre_asociado", ""), valor=row["valor"], empresa=row.get("empresa", ""), concepto=row["concepto"], categoria_final="LIBRANZAS", subcategoria_final="OCCIDENTE LIBRANZA", estado_validacion="OK" if str(row.get("estado_erp")).upper() == "ACTIVO" else row.get("estado_erp", "OK"), observaciones=row.get("observaciones"), fila_origen=idx + 1)
            session.add(reg)
        for idx, row in enumerate(rows_no_facturables):
            reg = NominaRegistroNormalizado(archivo_id=archivo.id, fecha_creacion=datetime.now(), mes_fact=mes, año_fact=anio, cedula=row["cedula"], nombre_asociado=row.get("nombre_asociado", ""), valor=row["valor"], empresa=row.get("empresa", ""), concepto=row["concepto"], categoria_final="LIBRANZAS", subcategoria_final="OCCIDENTE LIBRANZA", estado_validacion=row.get("estado_erp", "ADVERTENCIA"), observaciones=row.get("observaciones"), fila_origen=len(rows_facturables) + idx + 1)
            session.add(reg)
        await session.commit()
    except Exception as e:
        await session.rollback()
        raise HTTPException(status_code=500, detail=f"Error al guardar registros de Occidente Libranza: {str(e)}")
...
async def obtener_datos_occidente_libranza(mes: int = Query(...), anio: int = Query(...), session: AsyncSession = Depends(obtener_db)):
    """Devuelve datos LIBRANZA OCCIDENTE guardados para un mes/año."""
    stmt = select(NominaRegistroNormalizado).where(NominaRegistroNormalizado.subcategoria_final == "OCCIDENTE LIBRANZA", NominaRegistroNormalizado.mes_fact == mes, NominaRegistroNormalizado.año_fact == anio)
    try:
        result = await session.execute(stmt); registros = result.scalars().all()
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error al consultar datos de Occidente Libranza: {str(e)}")
