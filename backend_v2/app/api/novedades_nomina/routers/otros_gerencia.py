...
async def obtener_datos_otros_gerencia(mes: int = Query(...), anio: int = Query(...), session: AsyncSession = Depends(obtener_db)):
    from ....services.novedades_nomina.nomina_service import NominaService
...
async def buscar_empleado_otros_gerencia(cedula: str, db_erp = Depends(obtener_erp_db_opcional)):
