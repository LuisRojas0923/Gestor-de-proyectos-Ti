...
async def ejecutar_blindaje_estructural(conn):
    ...
    # 10. Seguimiento de Requisiciones de Personal (Gestión Humana)
    await safe_execute(
        conn,
        """
    ...
    await safe_execute(conn, "CREATE INDEX IF NOT EXISTS idx_candidatos_req_id ON candidatos_requisicion(requisicion_id)")
    await safe_execute(conn, "CREATE INDEX IF NOT EXISTS idx_candidatos_estado ON candidatos_requisicion(estado)")
    
    # Semillado preventivo
    await safe_execute(conn, "ALTER TABLE empresas_temporales ALTER COLUMN activo SET DEFAULT TRUE")
