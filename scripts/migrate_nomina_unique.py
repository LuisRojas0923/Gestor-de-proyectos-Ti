...
from backend_v2.app.database import engine
    ...
    try:
        async with engine.begin() as conn:
            ...
            await conn.execute(text("""
                DELETE FROM nomina_excepcion_historial a USING (
                    SELECT MAX(id) as id, excepcion_id, mes, anio
                    FROM nomina_excepcion_historial 
            ...
            try:
                await conn.execute(text("ALTER TABLE nomina_excepcion_historial DROP CONSTRAINT IF EXISTS uq_nomina_ex_historial"))
            except Exception as e:
                print("Nota: Constraint no existía o no se pudo borrar (esperado si es primera vez):", e)

            print("Aplicando constraint UNIQUE...")
            await conn.execute(text("""
                ALTER TABLE nomina_excepcion_historial 
                ADD CONSTRAINT uq_nomina_ex_historial UNIQUE (excepcion_id, mes, anio)
            """))
